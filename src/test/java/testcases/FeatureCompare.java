...
//  report.generateReport(); // email.sendemail(); // driver.close(); 
	...
	public Object[][] getTestData() throws Exception {
		final XSSFSheet sheet = ExcelReader.readExcelFile("FeatureCompare");
	...
	public void executeTestCase(String cell, int rowIndex) throws Exception {
		XSSFSheet sheet = ExcelReader.readExcelFile("FeatureCompare");
		if (cell.equalsIgnoreCase("FeatureTataAIG")) {
			Feature feature = new Feature();
			feature.beforetest();
			feature.test(rowIndex, sheet);
		} else if (cell.equalsIgnoreCase("CompareTataAIG")) {
			Compare compare = new Compare();
			compare.beforetest();
			compare.test(rowIndex, sheet);
		} 
